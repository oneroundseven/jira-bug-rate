...
let uedFEUsers = ['sheny', 'caixiaofeng', 'caojiarui', 'chenghao', 'fangxingxing', 'jiangcheng-a',
    ...
    'sunzhaowei', 'tanglu', 'yuyiming', 'huangyinxiang', 'miaoxiongtao'];
let issuetype = {
...
module.exports = {
    ...
    projects.join(comma) +')%20AND%20issuetype%20in%20('+
    issuetype.task.join(comma) +')%20AND%20status%20not%20in%20(Aborted)%20AND%20assignee%20in%20('+
    uedFEUsers.join(comma) +')%20AND%20fixVersion%3D"{{fixVersion}}"%20ORDER%20BY%20updatedDate',
    ...
    uedFEUsers.join(comma) +')%20AND%20fixVersion%3D"{{fixVersion}}"',
    users: uedFEUsers
}